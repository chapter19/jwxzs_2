...
class CollectFriendsFilter(django_filters.rest_framework.FilterSet):
    friend_name=django_filters.CharFilter(field_name='friend__name',lookup_expr='icontains',help_text='好友姓名')
    friend_username=django_filters.CharFilter(field_name='friend__username',lookup_expr='icontains',help_text='好友用户名(学号教号)')
    class Meta:
        model=CollectFriends
        fields=['friend_name','friend_username']
